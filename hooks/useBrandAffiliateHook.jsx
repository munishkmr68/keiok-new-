...
import { BrandAffiliateInitialValues } from '../src/services/Methods/validationSchema';
...
const useBrandAffiliateHook = (props) => {
    const section = 'brand_affiliate';
    ...
    const [formData, setFormData] = useState(BrandAffiliateInitialValues);
    ...
    const { isLoginLoading, validateCustomerToken, enrollApiErrorMsg, alluserData } = useSelector((state) => memoRizedSelector(state));
    const scrollToTop = () => {
    ...
    function _handelGooglePlace(results, section = null, formikMehtod = null) {
        if (results) {
            ...
            setCountryState((prv) => ({ ...prv, country: ADDRESS?.businessCountry, state: ADDRESS?.businessState }));
        }
    }
...
export default useBrandAffiliateHook;